...
`display.showCacheHit` is a separate opt-in that reports a different thing: **the share of the most recent request's input that came from the prompt cache**, e.g. `CacheHit 82.3%`. It is `cache_read / (input_tokens + cache_creation_input_tokens + cache_read_input_tokens)`. Counting the whole input matters on Anthropic-compatible endpoints that never report cache writes: with `cache_creation_input_tokens` stuck at 0, a denominator of cache reads plus cache writes would pin the rate at 100% forever. A request that only wrote cache shows `CacheHit 0.0%`, and a request with no cache activity — or a session that has not sent one yet — shows a dim `CacheHit --` placeholder instead of dropping the segment, so the slot stays put from the first frame. It renders on the first line as the `cacheHit` segment, so `projectLineOrder` can move it; set `display.cacheHitPlacement` to `stats` to put it on the appended stats line next to the compaction count instead.
