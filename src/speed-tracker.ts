...
import {
  ensureReceiver, isTelemetryEnabled, readSettingsOtelEndpoint, resolveOtelEndpoint,
} from './otel/lifecycle.js';
 ...
 */
export function getMeasuredTps(
  stdin: StdinData,
  config?: HudConfig,
  ...
  if (!otel || otel.mode === 'off') return null;
  if (!isTelemetryEnabled(process.env)) return null;
  ...
  if (otel.autoStart) {
    ...
    const endpoint = resolveOtelEndpoint(
      process.env,
      readSettingsOtelEndpoint(homeDir, stdin.cwd),
    );
