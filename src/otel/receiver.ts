...
function readAttributes(record: Record<string, unknown>): Record<string, unknown> {
  const attrs: Record<string, unknown> = {};
  const list = record.attributes;
  if (!Array.isArray(list)) return attrs;
...
export function extractSamples(payload: unknown): ExtractedSample[] {
  const out: ExtractedSample[] = [];
  if (typeof payload !== 'object' || payload === null) return out;

  const resourceLogs = (payload as Record<string, unknown>).resourceLogs;
  if (!Array.isArray(resourceLogs)) return out;

  for (const resourceLog of resourceLogs) {
    const scopeLogs = (resourceLog as Record<string, unknown> | null)?.scopeLogs;
    if (!Array.isArray(scopeLogs)) continue;

    for (const scopeLog of scopeLogs) {
      const records = (scopeLog as Record<string, unknown> | null)?.logRecords;
      ...
      for (const record of records) {
        ...
        if (body?.stringValue !== 'claude_code.api_request') continue;
 ...
 */
function capSampleFile(filePath: string): void {
  let size: number;
  try {
  ...
  if (size <= MAX_SAMPLE_FILE_BYTES) return;
...
export function appendSamples(homeDir: string, samples: ExtractedSample[]): void {
...
export function pruneOldSamples(homeDir: string, retentionDays: number, now: number): void {
  const dir = getOtelDir(homeDir);
  let entries: string[];
  try {
  ...
  for (const file of survivors) {
    if (count <= MAX_SAMPLE_FILES && total <= MAX_SAMPLE_DIR_BYTES) break;
    try {
