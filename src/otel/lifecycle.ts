...
function endpointFromSettingsFile(filePath: string): string | null {
  try {
 ...
 */
export function readSettingsOtelEndpoint(homeDir: string, cwd?: string): string | null {
 ...
 */
function explicitPort(raw: string): number | null {
...
export function readPidRecord(filePath: string): PidRecord | null {
  ...
  if (typeof r.entry !== 'string' || r.entry === '') return null;
  if (typeof r.port !== 'number' || !Number.isInteger(r.port)) return null;

  return { pid: r.pid, entry: r.entry, port: r.port };
}

function readSpawnState(filePath: string): SpawnState | null {
  try {
    const parsed = JSON.parse(fs.readFileSync(filePath, 'utf8')) as unknown;
    if (typeof parsed !== 'object' || parsed === null) return null;
    ...
    if (typeof s.lastAttemptMs !== 'number' || !Number.isFinite(s.lastAttemptMs)) return null;
...
function writeSpawnState(filePath: string, state: SpawnState): void {
  try {
...
function clearSpawnState(filePath: string): void {
  try {
 ...
 */
export function resolveReceiverEntry(): string {
 ...
 */
export function isReceiverProcess(pid: number, entry: string): boolean {
 ...
 */
export function isPortInUse(port: number, timeoutMs = PORT_PROBE_TIMEOUT_MS): Promise<boolean> {
 ...
 * Tests await it.
 */
export async function ensureReceiver(options: EnsureReceiverOptions): Promise<void> {
  const { homeDir, endpoint, retentionDays } = options;
  if (!endpoint || !endpoint.loopback) return;
  ...
  if (await isPortInUse(endpoint.port)) return;
